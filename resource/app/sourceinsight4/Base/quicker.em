...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    {
		InsBufLine(hbuf, ln + 2,  " 				 版权所有 (C), 2012-2016, @szMyCom@")
    }
    else
    {
		InsBufLine(hbuf, ln + 2,  " 				 版权所有 (C), 2012-2016, XXXXXX(XX)有限公司")
    }
    InsBufLine(hbuf, ln + 3,  "")
    InsBufLine(hbuf, ln + 4,  " ******************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
...
{      
	...
	//str = cat("/*",str)  
	//str = cat(str,"*/")  
    szStructName = (Ask("请输入段注释:"))//toupper
	//	SetBufSelText (hbuf, str)  
	 /*取得用户名*/
    szMyName = getreg(MYNAME)
    if(strlen( szMyName ) == 0)
...
macro Get_C_FileName()//获取路径名称，方便CD
{
    hbuf = GetCurrentBuf()
...
macro GetFullDirName()//获取路径名称，方便CD
{
    hbuf = GetCurrentBuf()
    hbufClip = GetBufHandle("Clipboard")
    EmptyBuf(hbufClip)
    hbuf = GetBufName(hbuf)//获取全路径名
    dir_buf = GetDirName(hbuf)//获取路径名
    dir_buf = cat("cd \"",dir_buf)
    dir_buf = cat(dir_buf,"\"")
    AppendBufLine(hbufClip,dir_buf)
}

macro GetFullFileName()//获取全路径方便运行
{
    hbuf = GetCurrentBuf()
    hbufClip = GetBufHandle("Clipboard")
    EmptyBuf(hbufClip)
...
macro Auto_printf_var()//插入printf 调试复制区  
{      
